#!/usr/bin/env python3
"""Train on Yamanishi labels and evaluate on the old BindingDB-labeled table."""
...
def load_bindingdb_rows(
    ...
    for row in read_xlsx_sheet(workbook, "original"):
        if row.get("label") not in {"0", "1"}:
            continue
        cid = row["cid"].strip()
        out = {"pubchem_cid": cid, "uniprot_id": row.get("uniprot_id", "").strip(), "label": row["label"].strip()}
...
def main() -> None:
    ...
    for feature_set_name, columns in FEATURE_SETS.items():
        ...
        for model_name, model in candidate_models(args.seed).items():
            ...
            results.append(
                {
                    "Train Dataset": "yamanishi",
                    "Test Dataset": "old_bindingdb",
    ...
    metrics_path = args.results_dir / "yamanishi_train_bindingdb_eval_metrics.csv"
    ...
    manifest = {
        "seed": args.seed,
        "train_dataset": str(args.yamanishi),
        "test_dataset": "data/raw/old_PSB_Data.xlsx:original",
        ...
        "train_test_exact_pair_overlap": len(pair_keys(train_rows) & pair_keys(test_rows)),
        "feature_sets": FEATURE_SETS,
    }
    manifest_path = args.results_dir / "yamanishi_train_bindingdb_eval_manifest.json"
